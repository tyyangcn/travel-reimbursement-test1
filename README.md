# 差旅报销管理系统

## 项目简介

本项目是一个差旅报销管理系统，用于完成报销单、补录行程、差旅补助和费用分摊等业务。当前后端已实现报销单从草稿创建、修改到送审的主要流程。

单据状态定义：

- `0`：草稿，可编辑、删除和送审
- `1`：审批中，送审后只读
- `2`：已作废，只读

## 已实现功能

- 报销单分页查询、新增、详情查询、修改和送审
- 补录行程新增、查询、修改和删除
- 根据行程日期自动生成补助及补助日历
- 行程变更后重新计算补助金额
- 费用分摊保存、查询和删除
- 报销单补助总金额自动汇总
- 请求参数校验和全局异常处理
- 统一接口返回格式

## 技术栈

- Java 17
- Spring Boot 3.2.12
- MyBatis-Plus 3.5.12
- MySQL 8.4
- Maven 3.9
- Lombok
- Jakarta Validation

## 项目结构

```text
src/main/java/com/example/demo
├── common                 公共响应和异常处理
├── config                 MyBatis-Plus 等配置
└── reimburse
    ├── controller         接收 HTTP 请求
    ├── dto                接收前端请求参数
    ├── entity             数据库表对应实体
    ├── mapper             数据访问层
    ├── service            业务接口
    ├── service/impl       业务实现
    ├── support            业务辅助类
    └── vo                 返回给前端的数据
```

## 数据库

数据库名称：`travel_reimbursement`

主要数据表：

- `fk_reim_main`：报销单主表
- `fk_reim_trip`：补录行程表
- `fk_reim_allowance`：补助信息表
- `fk_reim_allowance_calendar`：补助日历明细表
- `fk_reim_allocation`：费用分摊表
- `fk_reim_operation_log`：操作日志表

## 本地运行

1. 安装并启动 MySQL 8.4，创建 `travel_reimbursement` 数据库。
2. 修改 `src/main/resources/application.properties` 中的数据库用户名和密码。
3. 使用 JDK 17 和 Maven 3.9 启动项目：

```bash
mvn spring-boot:run
```

4. 后端默认访问地址：

```text
http://localhost:8082
```

## 接口返回格式

```json
{
  "code": 200,
  "message": "操作成功",
  "data": {}
}
```

## 项目文档

- [后端接口文档](docs/接口文档.md)
- [后端测试报告](docs/测试报告.md)

## 作者

- GitHub：`123alien`
